...
(async () => {
  ...
  const vc = {
    ...
    "credentialStatus": {
      "id": "https://localhost:3001/api/credential-status-lists/tsakkboai5pbg69rvrz1zjkv#2",
      "type": "StatusList2021Entry",
      "statusListIndex": 2,
      "statusListCredential": "https://localhost:3001/api/credential-status-lists/tsakkboai5pbg69rvrz1zjkv",
  ...
  // const sigHashHex = Array.from(sigHash).map(b => b.toString(16).padStart(2, '0')).join('');
  console.log(`sigHash: ${sigHash}`)
  const sigHashHex = secp.utils.bytesToHex(sigHash);
  ...
  const signRes = await fetch(`https://api.mahola.xyz/api/v1/users/me/addresses/${addressId}:blind_sign`, {
    ...
    body: JSON.stringify({
      message: sigHashHex, // ここの形式はあってる？
    }),
  });
  // const { signature } = await signRes.json();
  const { r, s } = await signRes.json();
  console.log(`r: ${r}`);
  console.log(`r: ${BigInt("0x" + r)}`);
  console.log(`s: ${s}`);
  console.log(`r: ${BigInt("0x" + s)}`);
  // const r = signRes.data.r;
  // const s = signRes.data.s;
  const signature = new secp.Signature(BigInt("0x" + r), BigInt("0x" + s)).normalizeS().toCompactHex();
